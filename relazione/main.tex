...
\begin{itemize}
  \item \emph{custom} vs \emph{fast} (entrambe con \texttt{norm='ortho'}): le due
        implementazioni coincidono a meno della precisione di macchina (errori
...
DCT 1D, riga $1\times 8$  & fast vs traccia        & $\sim 1$ su $3^a$ cifra \\
DCT 2D, blocco $8\times 8$& fast vs traccia        & $\sim 1$ su $3^a$ cifra \\
DCT 2D, blocco $8\times 8$& custom vs fast         & $<10^{-10}$ \\
Round-trip IDCT2(DCT2(A)) & ricostruzione vs $A$   & $<10^{-10}$ \\
...
\begin{itemize}
  \item $d=0$: tutti i coefficienti del blocco vengono azzerati, inclusa la
        componente continua $c_{0,0}$; poiché la IDCT di un blocco nullo è
        nulla, l'immagine ricomposta diventa \emph{nera}. È invece con $d=1$
        (che conserva il solo $c_{0,0}$) che ogni blocco collassa al proprio
        valore medio, dando un'immagine a blocchi di grigio uniforme;
  \item $d=2F-2$: si conserva ogni coefficiente tranne $c_{F-1,F-1}$ (il
        singolo coefficiente in basso a destra, la frequenza massima).
\end{itemize}
...
\cmidrule{3-6}\cmidrule{8-11}
 & & 2 & 5 & 8 & 14 & & & 4 & 10 & 20 & 30 \\
\midrule
tenuti     & & 3 & 15 & 36 & 63 & & tenuti     & 10 & 55 & 190 & 255 \\
...
Le scacchiere hanno celle di $10\times10$ pixel. Poiché il lato della cella
($10$) non divide quello del blocco ($8$), i blocchi $8\times8$ non coincidono
con le celle; tuttavia la disposizione dei blocchi rispetto alla griglia delle
celle è periodica con periodo $\mathrm{lcm}(8,10)=40$. Di conseguenza tutte le
risoluzioni multiple di $40$ ($40,80,160,320,640$) contengono la stessa
distribuzione di blocchi e danno PSNR identici a $F=8$, come mostra la
Tabella~\ref{tab:checker}. Fa eccezione la $20\times20$: a $F=8$ viene di fatto
ritagliata a $16\times16$ e cattura solo una sotto-finestra del periodo, perciò i
suoi valori differiscono leggermente. Per $F=16$ e $F=32$ i periodi rilevanti
diventano $\mathrm{lcm}(16,10)=80$ e $\mathrm{lcm}(32,10)=160$: cambiano dunque
le risoluzioni che coincidono, ed è per questo che, nel set di dati completo, le
righe $F=16$/$F=32$ non risultano più identiche a tutte le risoluzioni. Infine la
$20\times20$ è più piccola del blocco $32\times32$, perciò la configurazione
$F=32$ non viene applicata (riportata come \texttt{--}). La
Figura~\ref{fig:checker_res} mostra le scacchiere a tre risoluzioni affiancate.

\begin{table}[H]
\centering
\caption{PSNR [dB] sulle scacchiere (celle $10\times10$) a varie risoluzioni,
$F=8$. Le risoluzioni multiple di $40=\mathrm{lcm}(8,10)$ danno valori identici;
la $20\times20$ (di fatto $16\times16$ a $F=8$) cattura solo una sotto-finestra
del periodo e fa eccezione.}
\label{tab:checker}
\begin{tabular}{lrrrr}
\toprule
...
di MSE, PSNR e percentuale di coefficienti mantenuti per ogni configurazione.
...
il $4.7\%$ dei coefficienti) si mantengono tra $22$ e $31$~dB, una qualità
ancora accettabile. Le Figure~\ref{fig:deer}--\ref{fig:tonypitony} mostrano le griglie
...
(sopra) e immagini sintetiche rappresentative (sotto).}
\label{tab:psnr}
\small
\begin{tabular}{lrrrrrr}
\toprule
& \multicolumn{4}{c}{$F=8$} & \multicolumn{2}{c}{$F=16$} \\
